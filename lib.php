...
/**
 * Filter custom menu items based on cohort membership.
 * Menu items with {id1,id2,id3} will only be shown to users in those cohorts.
 * 
 * @copyright 2026 Thomas Kautz <thomas.kautz@fh-joanneum.at>
 *
 * @param array $custommenuitems The custom menu items.
 * @return array Filtered custom menu items.
 */
function theme_boost_union_child_get_custom_menu_items($custommenuitems) {
    global $USER;

    // Skip for guests and non-logged-in users.
    if (!isloggedin() || isguestuser()) {
        return $custommenuitems;
    }

    // Get the user's system-level cohorts.
    ...
    foreach ($custommenuitems as $item) {
        ...
        $text = $item->text;
        $matches = [];
        // Extract cohort IDs from {id1,id2,id3} pattern.
        if (preg_match('/\{([^}]+)\}/', $text, $matches)) {
            $requiredcohortids = array_map('trim', explode(',', $matches[1]));
            $hasaccess = false;

            // Check if the user is in any of the required cohorts.
            ...
            // Skip if the user doesn't have access.
            if (!$hasaccess) {
                continue;
            }

            // Remove the cohort IDs from the displayed text.
            $item->text = trim(str_replace($matches[0], '', $text));
        }
        $filteredmenuitems[] = $item;
    }
